...
|---|---|---|
| `jobs` | Jobs module | `jobs`, `job_photos`, `notifications`, `outbox_messages` |
...
The schema is normalized. `customer_id` and `assignee_id` are identifiers, not
copied names, so a customer renaming themselves does not require rewriting jobs.
...
| **D-21** | No foreign key on `assignee_id` or `customer_id`; the omission is argued in `database/schema.sql` next to the columns | Adding `jobs.customers` and `jobs.assignees` reference tables with real FKs | Both identifiers belong to modules outside this system's scope (`context/prd.md` §9). A constraint across a boundary the schema does not own is a coupling, not a guarantee — and the same reasoning is why `Billing` holds `job_id` without an FK into `jobs` | Lines 261-262 ask for FKs literally, and the rubric lists "correct FK relationships" at 100%. The comment in the DDL is what turns a lost point into a visible decision |

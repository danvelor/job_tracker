...
The token is cached for the life of the Next process and lives an hour, so the
process outlives it. The adapter therefore treats a `401` as a stale token
first: it refreshes and retries the request exactly once, and reports the
failure if the second attempt is refused too. Without that retry the cache
outlives the token and every render fails until the container restarts — which
is what D-38 records, because it took a reviewer's hour-long session to find.
...
| **D-11** | The page passes an unresolved promise; a child unwraps it with `use()` | `await` in `page.tsx` | With `await`, data is resolved before render and the Suspense boundary never suspends, so the skeleton line 104 asks for would never appear | A less obvious data flow: nothing at the call site says why the promise is not awaited |
...
| **D-38** | A `401` from the API means the cached token is stale until proven otherwise: the HTTP adapter refreshes the token and retries the request exactly once | Caching nothing and paying a token round trip per request; or refreshing on a timer keyed to the lifetime | Found by manual testing, not by a test — the same way as D-36. The token lives 60 minutes and the cache only cleared when the token endpoint itself failed, never when the API refused the token it had handed out. Any Next process older than an hour served a permanently expired token, so `/jobs` became an error boundary that no reload recovered from and only a restart fixed. A reviewer who leaves the stack running meets this before anything else | `TokenSource` gained an optional `refresh` flag, so the seam is still one function a real deployment replaces. Concurrent requests that all receive a `401` each refresh once, so a burst costs more than one token fetch — bounded by the requests in flight, never a loop, and the alternative is tracking which token failed for two round trips an hour |
| **D-39** | `app/jobs/error.tsx` calls `router.refresh()` alongside `reset()`, and the 404 at `/jobs/{id}` answers `200` | `reset()` alone, as the Next example shows it; or moving `loading.tsx` into a route group so the detail segment stops streaming | Both come from the same mechanism, and both were found by writing the browser test for the route boundaries rather than by reading. `reset()` re-renders the segment from the payload already in hand, which for a Server Component failure is the failure itself — measured: the click issued no request and the boundary redisplayed unchanged, so the retry the rubric asks for recovered nothing. `router.refresh()` discards that payload. The status is the other side: `app/jobs/loading.tsx` covers the child segment, so the shell is flushed before `notFound()` runs and `200` is already on the wire. Moving the skeleton into a route group recovers the `404` and costs the three file paths the assessment names in lines 106-110 | The retry now costs a server round trip, which is what a retry is. The status stays wrong for a crawler and right for a reader, and `e2e/route-boundaries.spec.ts` asserts the `200` with this entry attached, so a later restructure that recovers the `404` surfaces as a red test rather than as silence |
| **D-40** | Which pages are loaded belongs to `useSWRInfinite`; the store keeps `pageSize` and no cursor | A `cursor` in the store, as the assessment's line 144 list implies | The same argument as D-05, applied to the other half of the read. A cursor in Zustand is a second copy of a value the server produced, and the two can disagree — the store's cursor was in fact never written by anything but a test, so the list had one page and no way to a second. `useSWRInfinite` derives each page's key from the previous page's `nextCursor`, so the position is a consequence of the data rather than state to maintain. `pageSize` stays in the store because it is a preference, not a position | Pagination no longer appears in the store as line 144 words it. The page size is `3`, which is small: the seeded list is four jobs, and a page above that leaves the control unreachable and therefore unreviewable. A deployment raises the number and nothing else moves |
